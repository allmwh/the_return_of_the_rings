...
class FastaExtreFilter:
    ...
    def fasta_extre_filter(
        self, input_fasta_path, output_fasta_path, gap_filter_num=20, max_filter_seq=3
    ):
        """
        ...
        # 迴圈拿index
        extre_index = self.__get_extre_index_max_cond(
            seq_array, gap_filter_num, max_filter_seq
        )

        # 刪掉有問題的
        self.__fasta_remove_extre_by_index(
            input_fasta_path, output_fasta_path, extre_index
        )
    ...
    def __get_pot_seq_by_seq_num(self, seq_array, filter_seq_num):
        ...
        for seq_index, element in enumerate(seq_array.T):
            ...
            vals, inverse, count = np.unique(
                element, return_inverse=True, return_counts=True
            )
    ...
    def __fasta_remove_extre_by_index(
        self, input_fasta_path, output_fasta_path, extre_index
    ):
        """
...
class SeqFilter:
    ...
    def length_filter_by_od_ident(
        self, od_ident, disorder_filter_length, order_filter_length
    ):
        # filter_length
        ...
        for _ in range(2):
            ...
            for i in nen0_check:
                if i:
                    ...
                    od_ident = (
                        od_ident[:start] + filed_od * (end - start) + od_ident[end:]
                    )
        ...
        for _ in range(2):
            ...
            for i in ned_check:
                if i:
                    ...
                    od_ident = (
                        od_ident[:start] + od_place * (end - start) + od_ident[end:]
                    )
